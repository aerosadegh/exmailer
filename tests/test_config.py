...
class TestConfigNormalization:
    ...
    def test_boolean_normalization(self, tmp_path):
        ...
        for input_val, expected in test_cases:
            config_file = tmp_path / f"bool_{str(input_val)}.json"
            config_file.write_text(
                json.dumps(
                    {
                        "domain": "test",
...
class TestAutoDiscovery:
    ...
    def test_auto_discovery_priority_order(self, tmp_path, clean_environment, monkeypatch):
        ...
        (mock_home / ".exmailer.json").write_text(
            json.dumps({"domain": "home-domain", "username": "home", "password": "p", "server": "s", "email_domain": "e"})
        )

        # Higher priority
        cwd_config = tmp_path / "exmailer.json"
        cwd_config.write_text(
            json.dumps({"domain": "cwd-domain", "username": "cwd", "password": "p", "server": "s", "email_domain": "e"})
        )
    ...
    def test_explicit_path_skips_auto_discovery(self, tmp_path, clean_environment):
        ...
        (tmp_path / "exmailer.json").write_text(
            json.dumps({"domain": "wrong-domain", "username": "u", "password": "p", "server": "s", "email_domain": "e"})
        )

        explicit_config = tmp_path / "explicit.json"
        explicit_config.write_text(
            json.dumps({"domain": "correct-domain", "username": "u", "password": "p", "server": "s", "email_domain": "e"})
        )
